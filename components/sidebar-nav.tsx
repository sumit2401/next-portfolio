        ...
        <div className="flex flex-col space-y-6 flex-1">
          {navItems.map((item, index) => (
            ...
            >
              ...
              {currentSection === index + 1 && (
                <motion.div
                  className="absolute -left-3 top-1/2 -translate-y-1/2 w-0.5 h-6 bg-teal-400 rounded-r-full"
                  initial={{ scaleY: 0 }}
                  animate={{ scaleY: 1 }}
              ...
              {/* Tooltip */}
              <div className="absolute left-full ml-4 top-1/2 -translate-y-1/2 bg-black/95 text-white px-3 py-2 rounded-lg text-sm whitespace-nowrap opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none border border-white/10 shadow-lg">
                {item.label}
              </div>
            </motion.button>
          ))}
